...
#define USE_USB3PHY_FOR_PCIE	BIT(0)
...
static int ti_pipe3_power_on(struct phy *x)
{
	...
	if (phy->phy_power_syscon) {
		...
		if (phy->flags & USE_USB3PHY_FOR_PCIE)
...
static int ti_pipe3_init(struct phy *x)
{
	...
	if (phy->flags & USE_USB3PHY_FOR_PCIE) {
		val = ti_pipe3_readl(phy->phy_rx, PHY_RX_ANA_PRGRAMMABILITY_REG);
		val |= MEM_EN_PLLBYP;
		ti_pipe3_writel(phy->phy_rx, PHY_RX_ANA_PRGRAMMABILITY_REG, val);
...
static int ti_pipe3_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_bool(node, "ti,usb3_as_pcie"))
		phy->flags |= USE_USB3PHY_FOR_PCIE;
