...
static int omap_hsmmc_set_power(struct device *dev, int power_on, int iov)
{
	...
	if (host->pbias) {
		...
		if (ret) {
			dev_err(mmc_dev(mmc), "pbias set voltage failed\n");
...
static int __omap_hsmmc_disable_boot_regulator(struct regulator *reg)
{
	int ret;
...
static int omap_hsmmc_disable_boot_regulator(struct omap_hsmmc_host *host)
{
	struct mmc_host *mmc = host->mmc;
	int ret;
	 ...
	 */
	ret = __omap_hsmmc_disable_boot_regulator(mmc->supply.vmmc);
	if (ret) {
		dev_err(host->dev, "disable boot on vmmc reg fail\n");
		return ret;
	}

	ret = __omap_hsmmc_disable_boot_regulator(mmc->supply.vqmmc);
	if (ret) {
		dev_err(host->dev, "disable boot on vmmc_aux reg fail\n");
		return ret;
	}

	ret = __omap_hsmmc_disable_boot_regulator(host->pbias);
	if (ret) {
		dev_err(host->dev, "disable boot on pbias reg fail\n");
		return ret;
	}
...
static int omap_hsmmc_reg_get(struct omap_hsmmc_host *host)
{
	...
	ret = omap_hsmmc_disable_boot_regulator(host);
	if (ret)
		return ret;
